...
export default function UAVCercaEB() {
  const route = useRoute<RouteProp<RootStackParamList, 'UAVCercaEB'>>();
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const { anomalia } = route.params;

  const [mostrarMapa, setMostrarMapa] = useState(true);
  const [coordenadaUAV, setCoordenadaUAV] = useState<{ latitud: number; longitud: number } | null>(null);

  useEffect(() => {
    const obtenerCoordenadas = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/coordenadasUAV`);
        setCoordenadaUAV({
        ...
        Alert.alert('❌ No se pudo obtener la ubicación del UAV');
      }
    };
  ...
  const injectedJS = coordenadaUAV
    ? `
    const uav = [${coordenadaUAV.latitud}, ${coordenadaUAV.longitud}];
    const anomalia = [${anomalia.latitud}, ${anomalia.longitud}];

    const map = L.map('map').setView(anomalia, 17);
    ...
    L.marker(uav, { icon: customIcon }).addTo(map).bindPopup("📍 UAV").openPopup();
    L.marker(anomalia, { icon: customIcon }).addTo(map).bindPopup("⚠️ Anomalía");

    const polyline = L.polyline([uav, anomalia], {
      color: '#C30E2E',
      weight: 4,
      opacity: 0.9,
      dashArray: '6, 4',
    ...
    const dLat = toRad(anomalia[0] - uav[0]);
    const dLon = toRad(anomalia[1] - uav[1]);
    const a = Math.sin(dLat/2) ** 2 + Math.cos(toRad(uav[0])) * Math.cos(toRad(anomalia[0])) * Math.sin(dLon/2) ** 2;
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
    const distancia = (R * c).toFixed(1);

    const midLat = (uav[0] + anomalia[0]) / 2;
    const midLon = (uav[1] + anomalia[1]) / 2;
      ...
      {mostrarMapa ? (
        ...
        <>
          ...
          <View style={styles.botonRow}>
            <TouchableOpacity style={styles.botonAzul} onPress={accionarDispersor}>
              <Text style={styles.botonTexto}>Accionar dispersor</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.botonRojo} onPress={detenerDispersor}>
              <Text style={styles.botonTexto}>Detener dispersor</Text>
            </TouchableOpacity>
          </View>

          <View style={styles.botonRow}>
            <TouchableOpacity style={styles.botonVerde} onPress={terminarAtencion}>
